...
export async function configurationHook(ctx: Context, next: () => Promise<any>) {
  try {
    ...
    const resultCustomFieldExists = await salesforceConfigurationService.getFielsOrder(http);
    const nameField = resultCustomFieldExists.data.fields.filter((field: any) => field.name === 'Order_Status__c');
    const configurationService = new ConfigurationService();
    await configurationService.proccessConfiguration(parameterList.get('ACCESS_TOKEN_SALEFORCE') || '', ctx, parameterList, nameField.length);
