...
export async function abandonedCartHook(ctx: Context, next: () => Promise<any>) {
  ...
  try {
    ...
    const userSalesforce = await salesforceClient.getUser(args.email, parameterList.get(ACCESS_TOKEN_SALEFORCE) || '');
    ...
    const resultCreateOpportunity = await salesforceOpportunity.createOpportunity(args, parameterList, userSalesforceId, http);
    const opportunityService = new OpportunityService();
    const resultProcessOpportunity = await opportunityService.processOpporunity(args, resultCreateOpportunity.data.id, parameterList.get(ACCESS_TOKEN_SALEFORCE) || '', parameterList, ctx.vtex.authToken, ctx.vtex.account);
