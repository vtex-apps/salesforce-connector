...
jest.mock('../../service/SalesforceClientService', () => jest.fn().mockImplementation(() => mockSalesforceClientService));
jest.mock('../../service/SalesforceOpportunityService', () => jest.fn().mockImplementation(() => mockSalesforceOpportunityService));
jest.mock('../../service/OpportunityService', () => jest.fn().mockImplementation(() => mockOpportunityService));
...
describe('abandonedCartHook', () => {
  ...
  test('Success execute abandoned cart', async () => {
    ...
    mockSalesforceClientService = {
      getUser: jest.fn().mockResolvedValue(Result.TaskOk({ records: [{ Id: 'id' }] } )),
    }
    mockSalesforceOpportunityService = {
      createOpportunity: jest.fn().mockResolvedValue(Result.TaskOk({ id: 'id' })),
    }
    mockOpportunityService = {
      processOpporunity: jest.fn().mockResolvedValue(Result.TaskOk({ data: 'data' })),
    }
