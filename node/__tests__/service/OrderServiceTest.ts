  ...
  describe('processOrder', () => {
    test('create order in Salesforce', async () => {
      mockSalesforceOrderService = {
        ...
        associateOrderAndProduct: jest.fn().mockResolvedValue(Result.TaskOk({}))
      }
      ...
      const response = await service.processOrder(order, "clientSalesforceId", "access_token", parameterList, "authToken", "accountVtex");
    ...
    test('Failure to associate order and product in Salesforce', async () => {
      mockSalesforceOrderService = {
        ...
        createProduct: jest.fn().mockResolvedValue(Result.TaskOk({ id: 'id' })),
        createPricebookEntry: jest.fn().mockResolvedValue(Result.TaskOk({ id: 'id' })),
        associateOrderAndProduct: jest.fn().mockResolvedValue(Result.TaskError('error')),
      }
      mockMasterDataService = {
        saveUpdateOrder: jest.fn().mockResolvedValue(Result.TaskOk({})),
        saveUpdatePriceBookEntry: jest.fn().mockResolvedValue(Result.TaskOk({}))
    ...
    test('Flow when a product exists in Salesforce', async () => {
      ...
      mockMasterDataService = {
        saveUpdateOrder: jest.fn().mockResolvedValue(Result.TaskOk({})),
        getPriceBookEntry: jest.fn().mockResolvedValue(Result.TaskOk([{ id: 'id', priceBookEntryId: 'priceBookEntryId' }])),
      }
      const response = await service.processOrder(order, "clientSalesforceId", "access_token", parameterList, "authToken", "accountVtex");
      expect(response).toEqual({
        status: 200,
