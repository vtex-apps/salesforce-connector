...
describe('SalesforceOrderService', () => {
  ...
  describe('createOrder', () => {
    test('Create order in Saleforce', async () => {
      ...
      const response = await service.createOrder(order, '', mockAxiosInstance, parameterList);
  ...
  describe('getProductByExternalId', () => {
    test('Get product by external id in Salesforce', async () => {
      ...
      const response = await service.getProductByExternalId('', mockAxiosInstance);
      expect(response).toEqual({
        status: 200,
        message: 'OK',
        data: undefined
  ...
  describe('createPricebookEntry', () => {
    test('Create pricebookEntry in Salesforce', async () => {
      jest.spyOn(mockAxiosInstance, 'post').mockResolvedValue({ status: 201 });
      const response = await service.createPricebookEntry('', '', item, mockAxiosInstance);
      expect(response).toEqual({
        status: 200,
        message: 'OK',
        data: undefined
  ...
  describe('associateOrderAndProduct', () => {
    test('Create associate order and product in Salesforce', async () => {
      jest.spyOn(mockAxiosInstance, 'post').mockResolvedValue({ status: 201 });
      const response = await service.associateOrderAndProduct('', '', item, mockAxiosInstance);
      expect(response).toEqual({
        status: 200,
        message: 'OK',
        data: undefined
  ...
  describe('updateUnitPricePricebookEntry', () => {
    test('Update unit price pricebookEntry in Salesforce', async () => {
      ...
      const response = await service.updateUnitPricePricebookEntry('', item, mockAxiosInstance);
      expect(response).toEqual({
        status: 200,
        message: 'OK',
        data: undefined
    ...
    test('Error to update unit price pricebookEntry in Salesforce', async () => {
      jest.spyOn(mockAxiosInstance, 'patch').mockRejectedValue(new Error('Request failed with status code 400'));
      const response = await service.updateUnitPricePricebookEntry('', item, mockAxiosInstance);
      expect(response).toEqual({
        status: 500,
  ...
  describe('updateStatusOrder', () => {
    test('Update status order in Salesforce', async () => {
      jest.spyOn(mockAxiosInstance, 'patch').mockResolvedValue({ status: 200 });
      const response = await service.updateStatusOrder('', '', mockAxiosInstance);
      expect(response).toEqual({
        status: 200,
        message: 'OK',
        data: undefined
    ...
    test('Error to update status order in Salesforce', async () => {
      ...
      expect(response).toEqual({
        status: 500,
        message: 'an error occurred when updating updateStatusOrder in salesforce',
        data: new Error('Request failed with status code 400')
      });
    });
  });
