...
import { CODE_STATUS_200, CODE_STATUS_201, CODE_STATUS_204, ENTITY_ORDER_V2, ENTITY_PARAMETER_V2, ENTITY_PRICEBOOKENTRY_V2 } from "../utils/constans";
...
export default class MasterDataService {
  ...
  public getParameters = async (account: string, http: AxiosInstance): Promise<Result> => {
    try {
      const { data, status } = await http.get(`http://${account}.myvtex.com/api/dataentities/${ENTITY_PARAMETER_V2}/search?_fields=id,parameterValue`)
  ...
  public getPriceBookEntry = async (productId: string, account: string, http: AxiosInstance): Promise<Result> => {
    try {
      const { data, status } = await http.get(`http://${account}.myvtex.com/api/dataentities/${ENTITY_PRICEBOOKENTRY_V2}/search?_fields=_all&_where=id=${productId}`)
      if (status === 200 && validateResponse(data)) {
        return Result.TaskOk(data);
      } else {
