...
export default class OrderService {
  public processOrder = async (order: OrderVtexResponse, clientSalesforceId: string, access_token: string, parameter: ParameterList, authToken: string, accountVtex: string): Promise<Result> => {
    try {
      ...
      const resultOrder = await salesforceOrderService.createOrder(order, clientSalesforceId, http, parameter);
      if (!resultOrder.isOk()) {
      ...
      const resultSave = await masterDataService.saveUpdateOrder(orderSF, accountVtex, httpVTX);
      if (!resultSave.isOk()) {
      ...
      for (let i = 0; i < order.items.length; i++) {
        ...
        if (itemsFound.records.length === 0) { // Product not exists
          ...
          const resultCreateOrderItem = await salesforceOrderService.associateOrderAndProduct(orderSF.idSfc, pricebookEntry.priceBookEntryId, item, http);
          ...
          //create relation order-product
          const resultCreateOrderItem = await salesforceOrderService.associateOrderAndProduct(orderSF.idSfc, priceBookEntryId, item, http);
          if (!resultCreateOrderItem.isOk()) {
            console.log(resultCreateOrderItem.message);
            console.log(resultCreateOrderItem.data);
            return Result.TaskError(resultCreateOrderItem.message);
